...
impl ResidualCapture {
    ...
    pub fn metal_decode(
        ...
        let dir = run_with_dump_dir(ENV_DECODE_DUMP_LAYERS, || {
            let _ = backend.decode_token(&layers, &dec_x, hidden, intermediate, layers[0].num_q_heads * layers[0].head_dim, layers[0].num_kv_heads * layers[0].head_dim, layers[0].num_q_heads, layers[0].num_kv_heads, layers[0].head_dim, layers[0].rope_base);
        })?;
